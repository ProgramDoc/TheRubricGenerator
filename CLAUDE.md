...
| Reviewer selects the assessed outcome(s) | **Per-outcome fan-out**: the run-create modal's "Appraise several outcomes per paper" panel calls `POST /api/quality-appraisal/extract-outcomes` (3 cr/paper, `backend/outcomes.py:extract_outcomes`), shows a per-paper checklist with a free-text add, and posts `paper_outcomes: {paper_id: [outcome_dict]}` → `quality_appraisal_runs.paper_outcomes_json`. Each selected outcome becomes one result row carrying `outcome_id` + `outcome_json`. Cost `36 + 21 × (k−1)` via `paper_charge()`. **Legacy single override** still works: the per-paper "Override outcome" text input → `outcome_overrides_json`, used when no structured outcomes are supplied. Either way the assessed outcome lands on `quality_appraisal_results.assessed_outcome` (the auto-pick stays in `primary_outcome`), and `rob2` / `rob2_crossover` / `rob2_cluster` get a Domain 1 prompt reminder that randomization is per-trial, not per-outcome. Grid shows a green outcome chip + the assessed outcome column; detail modal names the outcome in its subtitle. |
| Add a new risk-of-bias tool (…) | New `backend/rob_tools/<tool>.py` exposing `run(pdf_bytes, fields, classification, primary_outcome, progress)` and `prompt_catalog()`, then register in `backend/quality_appraisal.py:STUDY_TYPE_REGISTRY` + `_TOOL_RUNNERS`. Tools that need per-estimate iteration (like QUADAS-3) also expose `extract_estimates(pdf_bytes, fields)` and register in `_ESTIMATE_EXTRACTORS`; the registry entry sets `supports_estimates=True` so `appraise_paper` builds one `_Unit` per estimate instead of one per outcome. Tools whose output is not a GRADE input (like AMSTAR-2) set `skip_grade=True` so `appraise_paper` skips indirectness / imprecision / `compute_grade` |
...
| **Master agent atlas (taxonomy + classification + extraction + all appraisal/synthesis agents)** | [docs/shareable/study_taxonomy_master_shareable.md](docs/shareable/study_taxonomy_master_shareable.md) — unified 34-type study taxonomy (OGAI v1.9 ∪ annotator v2.1), full OGAI classification rubric (primacy rules, 11 design features, confusion pairs) + deployed classify/prefill prompts verbatim, three-layer extraction catalog, per-agent digests of every RoB tool / reporting guideline / GRADE domain / synthesis agent with companion-doc links. Deployed vs reference methodology is tagged in plain text. Digests in §§6.1/6.6/6.8/7 are the current only sharable coverage of RoB 2 parallel, QUADAS-3, AMSTAR-2, and the guideline checkers. Dev companion: [docs/development/study_taxonomy_master_dev.md](docs/development/study_taxonomy_master_dev.md) |
| **Docs: shareable vs development** | `docs/shareable/*_shareable.md` is the methodology as a reader on another stack should implement it — framework-free, **no revision history, no repo-internal references**. `docs/development/*_dev.md` is its internal companion: revision notes, links to the backing modules, and implementation status. A breaking change is logged in the dev doc *and* stated inline in the shareable doc where the contract is described. Older full internal references live at `docs/*_reference.md`. Convention enforced by the `shareable-methodology-doc` skill |
| Modify GRADE indirectness logic | `backend/indirectness.py` (PICO subdomains, severity decision tree, prompts) + `backend/quality_appraisal.py:compute_grade` (combines RoB + indirectness + imprecision downgrades). **Sharable cross-platform methodology**: [docs/shareable/quality_appraisal_grade_shareable.md](docs/shareable/quality_appraisal_grade_shareable.md) §5 |
| Modify GRADE imprecision logic | `backend/imprecision.py` (CI / N / events / fragility subdomains, severity decision tree, prompts) + `backend/quality_appraisal.py:compute_grade` (combines RoB + indirectness + imprecision downgrades). **Sharable cross-platform methodology**: [docs/shareable/quality_appraisal_grade_shareable.md](docs/shareable/quality_appraisal_grade_shareable.md) §6 |
| Modify the GRADE ladder / RoB→downgrade mapping | `backend/quality_appraisal.py` — `GRADE_LEVELS` + `_grade_index` + `_rob_downgrade` + `compute_grade` (lines ~309-445); initial certainty per design lives in `STUDY_TYPE_REGISTRY`. Branch order in `_rob_downgrade` is load-bearing (the five RoB instruments' vocabularies overlap on `Low`/`High`). **Sharable cross-platform methodology**: [docs/shareable/quality_appraisal_grade_shareable.md](docs/shareable/quality_appraisal_grade_shareable.md) (§§1-3, 8). Rating-up (large effect / dose-response / residual confounding) is **not implemented** — §13 of that doc reserves the section |
| Body-of-evidence GRADE (the **GRADE agent**) | **Not on `main`** — implemented on branch `claude/grade-agent` (`backend/synthesis/grade.py`: `grade_body()`, weighted RoB-across-studies, I²/Q inconsistency, pooled-CI + OIS imprecision, Egger/trim-fill publication bias gated at k≥10, the three upgrade domains, absolute effects). Methodology: [docs/shareable/grade_certainty_shareable.md](docs/shareable/grade_certainty_shareable.md), with its input contract in [docs/shareable/pooling_meta_analysis_shareable.md](docs/shareable/pooling_meta_analysis_shareable.md). **Do not confuse it with the per-paper appraisal GRADE above** — RoB, indirectness, and imprecision are genuinely different logic there, not variants. A downgrade-only draft variant also exists at [docs/shareable/grade_certainty_downgrades_shareable.md](docs/shareable/grade_certainty_downgrades_shareable.md) (§5 rating-up is a placeholder); it is identical to the full doc wherever no upgrade fires, but under-rates non-randomized bodies that qualify for rating up — share the full doc unless the draft is specifically wanted |
...
**DB tables** (initialised from `QUALITY_APPRAISAL_TABLES_SQL` + idempotent `migrate_qa_columns(conn)` for the post-launch indirectness + imprecision columns): `quality_appraisal_runs` (incl. `target_pico_json` for the user-supplied PICO, `imprecision_thresholds_json` for MID benefit/harm, and `paper_outcomes_json` for the per-outcome fan-out), `quality_appraisal_results` (incl. `outcome_id`/`outcome_json` identifying the appraised outcome, `indirectness_json`/`overall`/`levels`/`explanation` and `imprecision_json`/`overall`/`levels`/`explanation`), `quality_appraisal_events`. All date columns use `created_at` / `completed_at` (no `timestamp` column per the SQLite compat-wrapper gotcha). Runs are soft-deleted via `deleted_at`.
...
**Per-estimate path** — `cfg.supports_estimates=True` makes `appraise_paper` build one unit per estimate (`build_estimate_units`). Classify + prefill + STARD run once per paper; QUADAS-3 + GRADE run once per estimate. Each estimate produces a separate `quality_appraisal_results` row (same `paper_id`, distinct `estimate_id`). Cost = 36 cr per (paper, estimate) unit. Refunds are also per-unit — `summary["estimates_errored"]` tells `run_batch` how many units to refund.

**Per-outcome path** — the sibling axis for treatment designs, and the default shape of the orchestrator. `appraise_paper` builds a list of `_Unit`s (`build_outcome_units` / `build_estimate_units`) and `_appraise_units` runs the RoB + indirectness + imprecision + GRADE pass once per unit, writing one `quality_appraisal_results` row each. Classify + prefill + the reporting-guideline check stay once per paper (the guideline lazily, so a paper whose every RoB call fails never pays for it). **The two axes are mutually exclusive per paper** — for diagnostic accuracy the estimate *is* the outcome axis, and `POST /runs` returns 400 for a paper carrying both, because charging happens at run-create time while study classification happens at run time. AMSTAR-2 papers collapse to one unit (the tool rates the review, not an outcome) and report `units_skipped` so the extra charged units refund. Supplying nothing yields exactly one unit and the pre-existing behaviour. Cost `36 + 21 × (k−1)` — classify/prefill/guideline are already paid for in the 36, so only RoB (~15) + indirectness (~3) + imprecision (~3) repeat; `paper_charge()` is the single source of truth, called by both the charge path in `main.py` and `refund_for()` in `run_batch` so they cannot drift. `MAX_OUTCOMES_PER_PAPER = 10`.
