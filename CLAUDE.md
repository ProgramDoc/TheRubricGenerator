...
| Reference: RoB 2 + ROBINS-I V2 + QUADAS-3 prompts + decision trees | `docs/quality_appraisal_rob_reference.md` — verbatim transcription of every signaling question, elaboration, and pure-Python decision tree from `prompt_catalog()` for all three tools (including ROBINS-I V2 cohort variants A/B + single-arm adaptation). For sharing without cloning. |
...
- **Diagnostic Accuracy → QUADAS-3 v1.2 + STARD 2015 + High initial GRADE** (per-estimate path; indirectness + imprecision skipped)
