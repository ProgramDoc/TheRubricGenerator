...
**Detail view** — each row in the results grid is clickable (📋 icon on the study cell, plus each RoB domain / CONSORT / GRADE cell). Opens a full-screen split modal: **PDF.js viewer on the left** (loaded from `/api/papers/{pid}/pdf`, canvas + text layer per page) and a scrollable **detail panel on the right** with Summary → RoB 2 (5 collapsible domains with per-signal answer + rationale) → CONSORT (grouped by section, ✓/✗/N-A) → GRADE (initial → updated with downgrade explanation and domain breakdown) → Extracted fields. Clicking any rationale or CONSORT evidence chip **searches the live PDF text layer** for the first ~80 chars of the quote (with a longest-matching word n-gram fallback for paraphrased quotes) and flash-highlights the match with a 3.5s yellow pulse. Quote-to-highlight is best-effort — we never asked the LLM for PDF coordinates, so the fallback may miss for heavily paraphrased quotes (toast surfaces the miss). Pure UI — no backend changes, works on every existing run. See [frontend/quality-appraisal.html](frontend/quality-appraisal.html) `openDetailModal`, `renderDetailPanel`, `loadDetailPdf`, `jumpToQuote`.
