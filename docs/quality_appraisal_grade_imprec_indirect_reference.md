...
Reference for the LLM prompts and Python decision-tree rules used by The Rubric Generator's Quality Appraisal AI for the **GRADE indirectness** and **GRADE imprecision** domains. 
...
Both modules expose `prompt_catalog()` which is surfaced via `GET /api/quality-appraisal/prompts` to every signed-in user. 
