...
4. In `appraise_paper`, when `cfg.get("supports_estimates")` is true the paper fans out into one assessment unit per estimate (`build_estimate_units`); `_appraise_units` runs classify + prefill + STARD **once per paper** and QUADAS-3 + GRADE **once per estimate**. Each estimate produces a separate `quality_appraisal_results` row (same `paper_id`, distinct `estimate_id`). Non-diagnostic designs use the same loop over an outcome axis instead — see `build_outcome_units`.
