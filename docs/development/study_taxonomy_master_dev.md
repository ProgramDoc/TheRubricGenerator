...
- **Taxonomy union decision:** canonical tree = OGAI v1.9's 32 consolidated types ∪ platform v2.1's Single-Arm Trial + Dose-Escalation Study = 34 types. Controlled Before-After enters as taxonomy-only (📐) — it is not in the deployed `_TAXONOMY` prompt. v1.9's consolidation principle (merge types that route identically) adopted as the documented rationale; prospective/retrospective cohort and case-control-family distinctions live in extraction fields, not the tree.
...
- **Status-tag convention introduced** (⚙ deployed / 📐 reference) so one document can carry the OGAI reference methodology and the deployed behavior without conflating them. Deployed prompts (classify, prefill) transcribed verbatim from `backend/annotator.py`.
