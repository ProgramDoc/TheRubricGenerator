...
### 2026-09-04 — Failed domains never read as clean; event-count N/A enforced in code; polarity-safe signal instruction

**What changed.** Four hardening fixes prompted by an external adversarial review.

1. **Failed indirectness / imprecision assessments are labelled, not silently clean.** A failed LLM assessment previously left `overall="none"` / 0 levels and the GRADE explanation claimed "no serious indirectness/imprecision detected". The failure still contributes 0 downgrade levels (never invent a finding), but the overall is now stored as `not_assessed`, the domain explanation says the assessment failed, and `compute_grade` (new `indirectness_assessed` / `imprecision_assessed` / `unassessed_note` kwargs) appends an explicit caveat — "was not assessed … may overstate certainty" — instead of a reassurance. The same caveat now marks diagnostic-accuracy (QUADAS) GRADE lines, whose indirectness + imprecision are deliberately deferred.
2. **Event-count N/A for continuous outcomes is enforced in code.** The prompt asks for `n_a`, but if the model returned a not-precise event-count judgement for a confirmed-continuous outcome it still counted toward severity while the UI displayed the cell as "N/A — continuous outcome". `imprecision.run` now forces the exclusion whenever `outcome_is_binary` resolves to `False`, annotating the rationale.
3. **Polarity-safe signaling instruction.** The shared prompt line "Answer N (or PN) when the paper gives enough information to rule out the problem" conflicted with questions where "yes" is the good answer (e.g. RoB 2's 1.1 random-sequence question), risking a Y/N reversal. Replaced across RoB 2, RoB 2 cross-over, RoB 2 Cluster, ROBINS-I V1 and V2 (and every doc transcription) with an answer-the-question-as-worded instruction that names the mixed polarity explicitly.
4. **Study-type routing tolerates formatting noise.** `dispatch` and `appraise_paper` normalize the classifier's `study_type` (whitespace + case) against the registry vocabulary before routing, so a valid type can no longer fall through to "unsupported" (skipped + refund) over a stray newline.

**Impact.** Logic + prompt change. Stored results are unaffected; new runs where an assessment fails now carry `not_assessed` and a caveated explanation (consumers displaying `indirectness_overall` should tolerate the new value — unknown labels already render as an em-dash badge in the app). RoB signal answers may shift on problem-phrased questions where the old instruction inverted them — that was the bug.
