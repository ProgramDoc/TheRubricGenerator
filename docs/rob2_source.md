# `backend/rob_tools/rob2.py` — Cochrane RoB 2 source

Full source of the RoB 2 quality-appraisal module after the conditional-signal NA fix (commit `8e6d245`).

See [`rob2_conditional_signal_na_fix.md`](rob2_conditional_signal_na_fix.md) for the narrative summary of what changed and why.
...
# Signaling-question response options.
#
# The LLM always answers from ``_BASIC`` = Y / PY / PN / N / NI. NA (Not
# Applicable) is never an LLM-facing option — it is set deterministically by
# ``_normalize_conditional_signals`` for conditional questions whose
# precondition is unmet, based purely on the predecessor's answer (no paper
# context required). Keeping NA out of the LLM's vocabulary eliminates a
# duplicated decision (LLM vs rules) and shortens the prompt.
#
# ``SIGNAL_OPTIONS`` is the union of all stored values that can appear in
# ``signals`` after normalization — the LLM-facing subset plus NA — and is
# used by external consumers (developer-view payload, test loops).
_BASIC = ("Y", "PY", "PN", "N", "NI")
...
# ─────────────────────────────────────────────
# Conditional-signal normalization
# ─────────────────────────────────────────────
# NA is set deterministically here, never by the LLM. The cribsheet branching
# rules are purely a function of predecessor signal answers (no paper context
# required), so once the LLM has answered the unconditional + applicable
# questions, the NA-for-skipped-questions assignment is mechanical.
#
# Each branch below mirrors the predecessor-checks already implemented in the
# corresponding ``rob2_domainN_judge`` function. The decision-tree functions
# don't read skipped slots when the predecessor short-circuits, so this pass
# only affects display + downstream inspection; the domain judgement is
# unchanged.


def _force_na(signals: dict[str, str],
              rationales: dict[str, str],
              qid: str,
              reason: str) -> None:
    """Force ``signals[qid]`` to NA + replace rationale.

    Idempotent: if ``signals[qid]`` is already NA (e.g., this pass runs twice
    or composes with another), the existing rationale is preserved.
    """
    if signals.get(qid) == "NA":
        return
    signals[qid] = "NA"
    rationales[qid] = f"Not applicable — {reason}."


def _normalize_conditional_signals(
    domain_id: int,
    signals: dict[str, str],
    rationales: dict[str, str],
) -> tuple[dict[str, str], dict[str, str]]:
    """Override conditional signals to NA when their precondition is unmet.

    Returns new dicts (input dicts are not mutated). Mirrors the cribsheet
    branching for domains 2, 3, and 4. Domains 1 and 5 have no conditional
    ...
    if domain_id == 2:
        # 2.3 applies only if 2.1 or 2.2 indicates awareness (Y/PY/NI).
        q21 = signals.get("2.1", "NI")
        q22 = signals.get("2.2", "NI")
        if _no(q21) and _no(q22):
            _force_na(signals, rationales, "2.3",
                      "2.1 and 2.2 were both N/PN (no awareness — Q2.3 does not apply)")
            _force_na(signals, rationales, "2.4", "2.3 is NA")
            _force_na(signals, rationales, "2.5", "2.4 is NA")
        else:
            q23 = signals.get("2.3", "NI")
            if not _yes(q23):
                _force_na(signals, rationales, "2.4",
                          f"2.3 was {q23} (Q2.4 only applies when 2.3 is Y/PY)")
                _force_na(signals, rationales, "2.5", "2.4 is NA")
            else:
                q24 = signals.get("2.4", "NI")
                if _no(q24) or q24 == "NA":
                    _force_na(signals, rationales, "2.5",
                              f"2.4 was {q24} (Q2.5 only applies when 2.4 is Y/PY/NI)")
        # 2.7 applies only if 2.6 was N/PN/NI (analysis was NOT appropriate).
        q26 = signals.get("2.6", "NI")
        if _yes(q26):
            _force_na(signals, rationales, "2.7",
                      f"2.6 was {q26} (Q2.7 only applies when 2.6 is N/PN/NI)")

    elif domain_id == 3:
        # 3.2 applies only if 3.1 was N/PN/NI (data NOT available for all).
        q31 = signals.get("3.1", "NI")
        if _yes(q31):
            _force_na(signals, rationales, "3.2",
                      f"3.1 was {q31} (Q3.2 only applies when 3.1 is N/PN/NI)")
            _force_na(signals, rationales, "3.3", "3.2 is NA")
            _force_na(signals, rationales, "3.4", "3.3 is NA")
        else:
            q32 = signals.get("3.2", "NI")
            if not _no(q32):
                _force_na(signals, rationales, "3.3",
                          f"3.2 was {q32} (Q3.3 only applies when 3.2 is N/PN)")
                _force_na(signals, rationales, "3.4", "3.3 is NA")
            else:
                q33 = signals.get("3.3", "NI")
                if _no(q33) or q33 == "NA":
                    _force_na(signals, rationales, "3.4",
                              f"3.3 was {q33} (Q3.4 only applies when 3.3 is Y/PY/NI)")

    elif domain_id == 4:
        # 4.3 applies only if NEITHER 4.1 nor 4.2 was Y/PY.
        q41 = signals.get("4.1", "NI")
        q42 = signals.get("4.2", "NI")
        if _yes(q41) or _yes(q42):
            which = "4.1" if _yes(q41) else "4.2"
            _force_na(signals, rationales, "4.3",
                      f"{which} was Y/PY (Q4.3-4.5 only apply when both 4.1 and 4.2 are N/PN/NI)")
            _force_na(signals, rationales, "4.4", "4.3 is NA")
            _force_na(signals, rationales, "4.5", "4.4 is NA")
        else:
            q43 = signals.get("4.3", "NI")
            if _no(q43) or q43 == "NA":
                _force_na(signals, rationales, "4.4",
                          f"4.3 was {q43} (Q4.4 only applies when 4.3 is Y/PY/NI)")
                _force_na(signals, rationales, "4.5", "4.4 is NA")
            else:
                q44 = signals.get("4.4", "NI")
                if _no(q44) or q44 == "NA":
                    _force_na(signals, rationales, "4.5",
                              f"4.4 was {q44} (Q4.5 only applies when 4.4 is Y/PY/NI)")

    return signals, rationales
...
_SYSTEM_PROMPT = (
    ...
    "Some questions are conditional on a previous answer (their text begins with 'If X to QY:'); "
    "answer those normally based on the paper — the system will mark them as not-applicable "
    "downstream if the precondition turns out to be unmet. "
...
def _assess_domain(pdf_bytes: bytes, domain: dict[str, Any],
    ...
    # Apply cribsheet branching: mark conditional questions NA when their
    # precondition is unmet. NA is set here deterministically from the
    # predecessor answers, never by the LLM. Decision trees route around
    # inapplicable questions, so the judgement does not change.
    signals, rationales = _normalize_conditional_signals(domain["id"], signals, rationales)
