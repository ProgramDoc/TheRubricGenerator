...
from backend.evidence_synthesis.pooling import pool_outcome as synthesis_pool_outcome
...
def api_agents_grade(body: dict = Body(default=None),
    ...
    endpoint for hybrid auto-indirectness). See backend/evidence_synthesis/grade_agent.py.
    """
    user = require_user(rubricgen_session, x_api_key)
    require_active_seat(user, "engineer")
    try:
